...
/**
 * @author Joe Portner This activity displays a list of remote apps that are
 *         available for a given Connection
 */
public class AppList extends SvmpActivity {
	private static String TAG = AppList.class.getName();

	public static final int REQUEST_REFRESHAPPS_QUICK = 200; // send a request
																// for the app
																// list, get
																// result, and
																// finish
	public static final int REQUEST_REFRESHAPPS_FULL = 201; // send a request
															// for the app list,
															// get result, and
															// finish
	public static final int REQUEST_STARTAPP_RESUME = 202; // resume the
															// activity after
															// returning
	public static final int REQUEST_STARTAPP_FINISH = 203; // if we return
															// without errors,
															// finish the
															// activity
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (actionBar != null) {
			...
			allApps = new TabListener<AppListFragment>(this, "allApps", AppListFragment.class);
			Tab tab = actionBar.newTab().setText(R.string.appList_actionBar_allApps).setTabListener(allApps);
			actionBar.addTab(tab);

			favorites = new TabListener<AppListFragment>(this, "favorites", AppListFragment.class);
			tab = actionBar.newTab().setText(R.string.appList_actionBar_favorites).setTabListener(favorites);
			actionBar.addTab(tab);
		}

		// if this was started by a desktop shortcut, just launch the requested
		// app
	...
	public boolean onOptionsItemSelected(MenuItem menuItem) {
		...
		case android.R.id.home:
			// if we launched this activity directly from a shortcut, start a
			// new ConnectionList activity
		...
		case R.id.refreshAppListQuick:
			...
			authPrompt(connectionInfo); // utilizes "startActivityForResult",
										// which uses this.sendRequestCode
			return true;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQUEST_REFRESHAPPS_QUICK || requestCode == REQUEST_REFRESHAPPS_FULL) {
			...
			// the user intentionally canceled the activity, and we are supposed
			// to finish this activity after resuming
			finish();
	...
	public void onClick_App(View view) {
		...
		openApp(appInfo, true); // open the app, resume the activity after
								// returning
	}

	// this can be triggered by clicking an app in this activity, or by clicking
