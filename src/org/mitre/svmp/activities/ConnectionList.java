...
public class ConnectionList extends SvmpActivity {
	...
	private static final int REQUEST_STARTVIDEO = 102; // opens
														// AppRTCVideoActivity
	private static final int REQUEST_CHANGEPASSWORD = 103; // opens
															// AppRTCChangePasswordActivity

	private List<ConnectionInfo> connectionInfoList;
	private ListView listView;
	private BroadcastReceiver receiver;
	private int sendRequestCode = REQUEST_STARTVIDEO; // used in the
														// "afterStartAppRTC"
														// method to determine
														// what activity gets
														// started
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// title has to be set here instead of in Manifest, for compatibility
		// with shortcuts
		...
		// register a BroadcastReceiver that will allow for triggered layout
		// refreshes
		// when a running SessionService stops, it sends this broadcast to
		// notify the ConnectionList to update its
		...
		// if we received an intent indicating which connection to open, act
		// upon it
		...
		//	int id = intent.getIntExtra("connectionID", 1);
			...
			if (connectionInfo != null) {
				...
				if (Utility.getPrefBool(this, R.string.preferenceKey_connection_useDesktopMode,
						R.string.preferenceValue_connection_useDesktopMode)) {
	...
	// queries the database for the list of connections and populates the
	...
	protected void populateLayout() {
		...
		// ArrayAdapter<ConnectionInfo>(getApplicationContext(),
		// android.R.layout.simple_list_item_1, connectionInfoList));
		listView.setAdapter(new ConnectionInfoArrayAdapter(this,
				connectionInfoList.toArray(new ConnectionInfo[connectionInfoList.size()]))); // use
																								// two-line
																								// list
																								// items
	}
	...
	public void onClick_Item(View view) {
		try {
			...
			// if we allow use of desktop mode, start the connection; otherwise
			// take us to this connection's app list
			if (Utility.getPrefBool(this, R.string.preferenceKey_connection_useDesktopMode,
					R.string.preferenceValue_connection_useDesktopMode)) {
				this.sendRequestCode = REQUEST_STARTVIDEO;
				authPrompt(connectionInfo);
	...
	// Context Menu handles long-pressing (prompt allows user to edit or remove
	// connections)
	@Override
	public void onCreateContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo) {
		if (v.getId() == R.id.connectionList_listView_connections) {
			...
			// if our preferences are set to allow use of "Desktop mode", enable
			// that context button
			if (Utility.getPrefBool(this, R.string.preferenceKey_connection_useDesktopMode,
					R.string.preferenceValue_connection_useDesktopMode))
			...
			// if this uses password authentication, add an option to change the
			// password
			...
			// if this connection is running, display the context option to
			// close it
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case 1: // Edit connection
			// start a ConnectionDetails activity for editing an existing
			// connection
			startConnectionDetails(connectionInfo.getConnectionID());
			break;
		case 2: // Remove connection
			// find the connectionID; if that connection is running, stop the
			// session service
			...
			// delete the connection info and repopulate the layout to reflect
			// changes
		...
		case 99: // Change password
			...
			break;
		case 100: // Stop
			stopService(new Intent(ConnectionList.this, SessionService.class)); // stop
																				// the
																				// service
																				// that's
																				// running
	...
	// starts a ConnectionDetails activity for editing an existing connection or
	// creating a new connection
	private void startConnectionDetails(int id) {
		// create explicit intent
		Intent intent = new Intent(ConnectionList.this, ConnectionDetails.class);

		// if the given ID is valid (i.e. greater than zero), we are editing an
		// existing connection
		if (id > 0)
	...
	private void startConnectionApp(ConnectionInfo connectionInfo, String pkgName, String apkPath) {
		Intent intent = new Intent(ConnectionList.this, AppList.class);
		intent.putExtra("connectionID", connectionInfo.getConnectionID());
		intent.putExtra("description", connectionInfo.getDescription());
